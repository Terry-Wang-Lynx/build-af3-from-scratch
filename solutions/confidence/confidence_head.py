...
class ConfidenceHead(nn.Module):
    ...
    def forward(
        ...
        for i in range(N_sample):
            ...
            if z_trunk.shape[-2] > 2000 and (not self.training):
                # CPU-offload pae/pde for very large proteins to keep GPU memory low.
                # 对于非常大的蛋白把 pae/pde offload 到 CPU，避免显存爆。
    ...
    def memory_efficient_forward(
        ...
        z_pair += self.linear_no_bias_d_wo_onehot(distance_pred.unsqueeze(-1))
        s_single, z_pair = self.pairformer_stack(s_trunk, z_pair, pair_mask)
