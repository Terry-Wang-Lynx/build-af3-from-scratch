...
from runtime.scatter_utils import scatter
...
def centre_random_augmentation(
    ...
    if mask is None:
        ...
        # ``keepdim=True`` on the denominator preserves the coordinate axis so
        # ``[..., 3] / [..., 1]`` broadcasts; without it, batched inputs like
        # ``[B, N_atom, 3]`` raise a broadcast error ([B,3] / [B]).
...
def tree_map(fn: Callable, tree: Any, leaf_type: type) -> Any:
    if isinstance(tree, dict):
        ...
        raise ValueError(f"tree_map: unsupported leaf type {type(tree).__name__}")
