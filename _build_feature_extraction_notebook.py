"""Build the read-only feature_extraction walkthrough notebook.
...
def main() -> None:
    ...
    nb.cells = [
        new_markdown_cell(
            "## 基本环境 · Basic setup\n\n"
            "本章是**只读 walkthrough**，不需要填任何 TODO。它把 `feature_extraction/` "
            "这套工程代码从 JSON 输入到模型 ready 张量的全流程拆给你看。读完你会"
            "对每个特征张量的来源、含义、形状心里有数。\n\n"
            "This chapter is a **read-only walkthrough** — no TODOs to fill. It "
            "walks `feature_extraction/` from the input JSON down to the tensor "
            "dict the trunk consumes, so you understand where each feature comes "
            "from."
        ),
        new_code_cell(
            "import os, sys\n"
        ...
        new_markdown_cell(
            ...
            "`feature_extraction/` 把 PDB / mmCIF / FASTA / MSA / 模板 …… "
            "杂七杂八的原始数据**翻译**成主干模型能直接吃的张量字典。它本身没有"
            "可学参数，纯数据工程，所以在 AF3 教学里我们把它当黑盒用即可。"
            "本笔记本带你走一遍数据流，让你知道:\n\n"
            "- 输入是什么形状的 JSON\n"
            "- 内部经过哪几步处理\n"
        ...
        new_markdown_cell(
            "## 0.1 输入 JSON\n\n"
            "我们仓库自带的示例是 `examples/example.json`，里面是一个 7r6r 单链蛋白 + "
            "预算好的 MSA 路径。这是用户提供 AF3 的最常见输入: 一段 `proteinChain` 加"
            "MSA / 模板等可选元数据。"
        ),
        new_code_cell(
            "import json, pprint\n"
        ...
        new_markdown_cell(
            "## 0.2 dataloader 入口\n\n"
            "`get_inference_dataloader(cfg)` 是顶层入口 —— 内部读 JSON、"
            "对每个 sample 调 `SampleDictToFeatures`、MSA featurizer、template "
            "featurizer，最后吐出 PyTorch `DataLoader`。\n\n"
        ...
        # --- Section 3: feature dict tour ---
        new_markdown_cell(
            "## 0.3 输出特征字典\n\n"
            "`get_inference_dataloader` 输出的 `data` 字典里只有一个 `input_feature_dict` "
            "是真正喂给模型的张量包。下面看一下里面有什么:"
        ),
        new_code_cell(
        ...
        new_markdown_cell(
            "## 0.4 atom_array\n\n"
            "另一个返回值 `atom_array` 是 biotite 的 `AtomArray`，结构数据的标准容器。"
            "推理结束写 CIF 时会用到它（把预测坐标写回 atom_array 再 dump）。"
        ),
        new_code_cell(
        ...
        new_markdown_cell(
            "## 章节小结\n\n"
            "本章你没有写任何代码，目的只是知道 `feature_extraction/` 把 JSON 翻译成什么。"
            "实际推理里 `Protenix.forward` 会先调 `relative_position_encoding.generate_relp`"
            "把 `relp` 加进字典，再交给主干。\n\n"
            "本章不挖 TODO 是有意为之: feature_extraction 是工程代码 (mmCIF 解析、"
            "MSA 处理、模板搜索等)，与 AF3 算法层关系不大，把时间花在那里收益不高。"
        ),
    ]

    out = os.path.join(SOL, "feature_extraction", "feature_extraction.ipynb")
